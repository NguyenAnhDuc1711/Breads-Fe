// Verification script for Task 004 (epic seo-sitemap-schema): spot-checks the
// 3-branch access classification on `app/(main)/posts/[postId]/page.tsx`
// (deleted -> 404/410, catch-all -> 200+noindex, public happy-path -> 200+
// no noindex+SSR content) against a real running server. This is a plain
// Node script, NOT a unit test — this repo has no test framework (no
// jest/vitest config, no `test` script in package.json), so this is run
// manually/ad-hoc per the task's Verification Checklist (SC-2, N=150 total).
//
// HOW TO RUN
// ----------
// 1. Start the app to verify against (dev or staging), e.g.:
//      npm run dev            # http://localhost:3000
// 2. Prepare a fixtures JSON file listing real postIds for each category
//    (see FIXTURES SHAPE below). These must be curated ahead of time from
//    the DB/admin panel — this script does not invent or discover postIds.
// 3. Run:
//      BASE_URL=http://localhost:3000 \
//      FIXTURES_FILE=./scripts/fixtures/post-access.sample.json \
//      node scripts/verify-post-page-access.mjs
//
//    BASE_URL defaults to http://localhost:3000 if omitted.
//    FIXTURES_FILE defaults to ./scripts/fixtures/post-access.sample.json.
//
// FIXTURES SHAPE (JSON)
// ----------------------
// {
//   "deleted": ["<postId>", ...],              // N=50, status=DELETED
//   "private": ["<postId>", ...],              // N=50, visibility!=PUBLIC (any status except DELETED)
//   "preAcceptPublic": ["<postId>", ...],       // N=30, status=PRE_ACCEPT + visibility=PUBLIC
//   "publicHappyPath": [                        // N=20, status=PUBLIC + visibility=PUBLIC (merge-gate case)
//     { "id": "<postId>", "contentSnippet": "<first ~30 chars of post.content>" },
//     ...
//   ]
// }
//
// A tiny sample fixtures file (empty arrays) is checked in at
// scripts/fixtures/post-access.sample.json purely to document the shape —
// replace it with real postIds (or point FIXTURES_FILE elsewhere) before
// running for real; empty categories are skipped with a warning, not a
// failure, so an empty sample file "passes" trivially.
//
// EXIT CODE
// ---------
// 0 if every checked case passed, 1 otherwise. A summary table is printed
// either way.
